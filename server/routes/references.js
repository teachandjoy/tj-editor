...
function rowToRef(row) {
  return {
    ...
    moduleIds: row.module_ids ? JSON.parse(row.module_ids) : undefined,
    topicIds: row.topic_ids ? JSON.parse(row.topic_ids) : undefined,
  };
}
...
router.post('/', (req, res) => {
  try {
    ...
    db.prepare('INSERT INTO "references" (id, style, type, authors, title, year, journal, volume, issue, pages, doi, url, accessed_date, edition, chapter, editors, publisher, city, offer_id, module_tag, module_ids, topic_ids) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)')
      .run(r.id, r.style || 'apa', r.type || 'article', r.authors || null, r.title, r.year || null, r.journal || null, r.volume || null, r.issue || null, r.pages || null, r.doi || null, r.url || null, r.accessDate || null, r.edition || null, r.chapter || null, r.editors || null, r.publisher || null, r.city || null, r.offerId || null, r.moduleTag || null, r.moduleIds ? JSON.stringify(r.moduleIds) : '[]', r.topicIds ? JSON.stringify(r.topicIds) : '[]');
...
router.put('/:id', (req, res) => {
  try {
    ...
    const r = req.body;
    db.prepare('UPDATE "references" SET style = ?, type = ?, authors = ?, title = ?, year = ?, journal = ?, volume = ?, issue = ?, pages = ?, doi = ?, url = ?, accessed_date = ?, edition = ?, chapter = ?, editors = ?, publisher = ?, city = ?, offer_id = ?, module_tag = ?, module_ids = ?, topic_ids = ? WHERE id = ?')
      .run(r.style ?? existing.style, r.type ?? existing.type, r.authors !== undefined ? r.authors : existing.authors, r.title ?? existing.title, r.year ?? existing.year, r.journal !== undefined ? r.journal : existing.journal, r.volume !== undefined ? r.volume : existing.volume, r.issue !== undefined ? r.issue : existing.issue, r.pages !== undefined ? r.pages : existing.pages, r.doi !== undefined ? r.doi : existing.doi, r.url !== undefined ? r.url : existing.url, r.accessDate !== undefined ? r.accessDate : existing.accessed_date, r.edition !== undefined ? r.edition : existing.edition, r.chapter !== undefined ? r.chapter : existing.chapter, r.editors !== undefined ? r.editors : existing.editors, r.publisher !== undefined ? r.publisher : existing.publisher, r.city !== undefined ? r.city : existing.city, r.offerId !== undefined ? r.offerId : existing.offer_id, r.moduleTag !== undefined ? r.moduleTag : existing.module_tag, r.moduleIds !== undefined ? JSON.stringify(r.moduleIds) : existing.module_ids, r.topicIds !== undefined ? JSON.stringify(r.topicIds) : existing.topic_ids, req.params.id);
