...
// GET /api/folders — RBAC filtered by accessible offer IDs
router.get('/', requireAuth, (req, res) => {
  try {
    ...
    if (accessibleIds === null) {
      // admin — return all
      return res.json(rows.map(rowToFolder));
    }

    // Filter: folders linked to accessible offers + their parent chain
    const offerFolders = rows.filter(f => f.offer_id && accessibleIds.has(f.offer_id));
    const offerFolderIds = new Set(offerFolders.map(f => f.id));
    // Include parent folders so the tree renders correctly
    const parentIds = new Set(offerFolders.map(f => f.parent_id).filter(Boolean));
    const filtered = rows.filter(f => {
      if (offerFolderIds.has(f.id)) return true;
      if (parentIds.has(f.id)) return true;
      return false;
    });
    res.json(filtered.map(rowToFolder));
  } catch (err) {
