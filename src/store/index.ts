...
import {
  apiGet, apiPost, apiPut, apiDelete, apiUploadFile, apiUploadFilePut,
...
export function useAppStore() {
  ...
  const [trashedTopics, setTrashedTopics] = useState<Topic[]>([]);

  const loadTrashedTopics = useCallback(async () => {
    try {
      const trashed = await apiGet<Topic[]>('/topics/trash');
      setTrashedTopics(trashed);
    } catch { /* ignore */ }
  }, []);

  const restoreTopic = useCallback(async (id: string) => {
    try {
      const restored = await apiPut<Topic>(`/topics/${id}/restore`, {});
      setTrashedTopics(prev => prev.filter(t => t.id !== id));
      setTopics(prev => [...prev, restored]);
    } catch { /* fallback */ }
  }, []);

  const permanentDeleteTopic = useCallback(async (id: string) => {
    try {
      await apiDelete(`/topics/${id}/permanent`);
      setTrashedTopics(prev => prev.filter(t => t.id !== id));
    } catch { /* fallback */ }
  }, []);
  ...
  const uploadMedia = useCallback(async (file: File, meta: { id: string; folderId?: string; offerId?: string; uploadedBy: string }): Promise<MediaAsset> => {
    const formData = new FormData();
    formData.append('file', file);
    formData.append('id', meta.id);
    if (meta.folderId) formData.append('folderId', meta.folderId);
    if (meta.offerId) formData.append('offerId', meta.offerId);
    formData.append('uploadedBy', meta.uploadedBy);
    formData.append('uploadedAt', new Date().toISOString().split('T')[0]);
    const result = await apiUploadFile<MediaAsset>('/media/upload', formData);
    setMedia(prev => [...prev, result]);
    return result;
  }, []);

  const replaceMedia = useCallback(async (id: string, file: File): Promise<MediaAsset> => {
    const formData = new FormData();
    formData.append('file', file);
    const result = await apiUploadFilePut<MediaAsset>(`/media/${id}/replace`, formData);
    setMedia(prev => prev.map(m => m.id === id ? result : m));
    return result;
  }, []);
  ...
  const assignEditor = useCallback(async (targetType: 'offer' | 'module' | 'topic', targetId: string, editorId: string, offerId?: string) => {
    if (targetType === 'offer') {
      let updatedEditors: string[] = [];
      setOffers(prev => prev.map(o => {
        if (o.id === targetId) {
          updatedEditors = [...new Set([...(o.assignedEditors || []), editorId])];
          return { ...o, assignedEditors: updatedEditors };
        }
        return o;
      }));
      try { await apiPut(`/offers/${targetId}`, { assignedEditors: updatedEditors }); } catch { /* fallback */ }
  ...
  const removeEditor = useCallback(async (targetType: 'offer' | 'module' | 'topic', targetId: string, editorId: string, offerId?: string) => {
    if (targetType === 'offer') {
      let updatedEditors: string[] = [];
      setOffers(prev => prev.map(o => {
        if (o.id === targetId) {
          updatedEditors = (o.assignedEditors || []).filter(e => e !== editorId);
          return { ...o, assignedEditors: updatedEditors };
        }
        return o;
      }));
  ...
  const assignCoordinator = useCallback(async (targetType: 'offer' | 'module' | 'topic', targetId: string, coordinatorId: string, offerId?: string) => {
    if (targetType === 'offer') {
      let updated: string[] = [];
      setOffers(prev => prev.map(o => {
        if (o.id === targetId) {
          updated = [...new Set([...(o.assignedCoordinators || []), coordinatorId])];
          return { ...o, assignedCoordinators: updated };
        }
        return o;
      }));
      try { await apiPut(`/offers/${targetId}`, { assignedCoordinators: updated }); } catch { /* fallback */ }
    } else if (targetType === 'topic') {
      let updated: string[] = [];
      setTopics(prev => prev.map(t => {
        if (t.id === targetId) {
          updated = [...new Set([...(t.assignedCoordinators || []), coordinatorId])];
          return { ...t, assignedCoordinators: updated };
        }
        return t;
      }));
      try { await apiPut(`/topics/${targetId}`, { assignedCoordinators: updated }); } catch { /* fallback */ }
    } else if (offerId) {
      let updatedModules: Offer['modules'] = [];
      setOffers(prev => prev.map(o => {
        if (o.id === offerId) {
          updatedModules = o.modules.map(m =>
            m.id === targetId ? { ...m, assignedCoordinators: [...new Set([...(m.assignedCoordinators || []), coordinatorId])] } : m
          );
          return { ...o, modules: updatedModules };
        }
        return o;
  ...
  const removeCoordinator = useCallback(async (targetType: 'offer' | 'module' | 'topic', targetId: string, coordinatorId: string, offerId?: string) => {
    if (targetType === 'offer') {
      let updated: string[] = [];
      setOffers(prev => prev.map(o => {
        if (o.id === targetId) {
          updated = (o.assignedCoordinators || []).filter(c => c !== coordinatorId);
          return { ...o, assignedCoordinators: updated };
        }
        return o;
      }));
      ...
      setTopics(prev => prev.map(t => {
        if (t.id === targetId) {
          updated = (t.assignedCoordinators || []).filter(c => c !== coordinatorId);
          return { ...t, assignedCoordinators: updated };
        }
        return t;
      }));
      ...
      setOffers(prev => prev.map(o => {
        if (o.id === offerId) {
          updatedModules = o.modules.map(m =>
            m.id === targetId ? { ...m, assignedCoordinators: (m.assignedCoordinators || []).filter(c => c !== coordinatorId) } : m
          );
          return { ...o, modules: updatedModules };
        }
        return o;
  ...
  return {
    ...
    loadTrashedTopics, restoreTopic, permanentDeleteTopic,
    addTemplate, updateTemplate, deleteTemplate,
    addReference, updateReference, deleteReference,
    addMedia, uploadMedia, replaceMedia, deleteMedia,
    addFolder, updateFolder, deleteFolder,
    addIdentity, updateIdentity, deleteIdentity,
    assignEditor, removeEditor, assignCoordinator, removeCoordinator,
    setOffers, setUsers,
