...
import { FileText, BookOpen, Image, BookMarked, Users, CheckCircle, Clock, AlertTriangle, Edit3, BarChart3, TrendingUp, Target, X, Settings } from 'lucide-react';
...
export default function DashboardPage() {
  const { currentUser, topics, offers, media, references, users, updateUser } = useApp();
  ...
  // Backend already filters by RBAC — topics and offers returned are only what user can access
  ...
  const generalStats = [
    ...
    { label: 'Ofertas activas', value: visibleOffers.length, icon: BookOpen, color: TJ.secondary },
  ...
  return (
    <div className="page-enter">
      ...
      <div className="p-3 md:p-6">
        ...
        {/* Offer tabs */}
        {(isAdmin || visibleOffers.length > 0) && (
        ...
        {isAdmin && (
          <div className="mb-4">
            <button onClick={() => setShowDashConfig(!showDashConfig)}
              className="flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-lg transition-colors"
              style={{ color: TJ.primary, background: showDashConfig ? 'rgba(27,75,133,0.08)' : 'transparent' }}>
              <Settings size={13} /> Configurar Dashboard por usuario
            </button>
            {showDashConfig && (
              <div className="mt-2 bg-white rounded-xl border p-4" style={{ borderColor: TJ.border }}>
                <div className="flex items-center gap-3 mb-3">
                  <label className="text-xs font-semibold" style={{ color: TJ.text }}>Usuario:</label>
                  <select className="text-xs border rounded-lg px-2 py-1.5" style={{ borderColor: TJ.border }}
                    value={configUserId} onChange={e => setConfigUserId(e.target.value)}>
                    <option value="">— Seleccionar —</option>
                    {users.map(u => <option key={u.id} value={u.id}>{u.name} ({u.role})</option>)}
                  </select>
                </div>
                {configUserId && (() => {
                  const targetUser = users.find(u => u.id === configUserId);
                  if (!targetUser) return null;
                  const cfg = (targetUser.dashboard_config || {}) as Record<string, boolean>;
                  const widgets = ['Temas totales', 'Ofertas activas', 'Multimedia', 'Referencias', 'Usuarios', 'Gráficas', 'Estado de Temas', 'Temas Recientes'];
                  return (
                    <div className="space-y-1.5">
                      <p className="text-xs mb-2" style={{ color: '#a8b8d8' }}>Widgets visibles para <strong>{targetUser.name}</strong>:</p>
                      {widgets.map(w => {
                        ...
                        return (
                          <label key={w} className="flex items-center gap-2 text-xs cursor-pointer">
                            <input type="checkbox" checked={visible}
                              onChange={() => {
                                const newCfg = { ...cfg, [key]: !visible };
                                updateUser(configUserId, { dashboard_config: newCfg } as any);
                              }}
                              className="accent-blue-700" />
                            <span style={{ color: TJ.text }}>{w}</span>
                          </label>
                        );
                      })}
                    </div>
