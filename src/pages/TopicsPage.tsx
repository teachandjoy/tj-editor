...
import { Search, Plus, FileText, Eye, Filter, X, ChevronDown, Upload, Trash2, RotateCcw, Archive } from 'lucide-react';
...
export default function TopicsPage() {
  const { currentUser, topics, offers, templates, addTopic, updateTopic, deleteTopic, trashedTopics, loadTrashedTopics, restoreTopic, permanentDeleteTopic } = useApp();
  ...
  const canDelete = hasPermission(currentUser, 'temas', 'eliminar');

  // Backend already filters by RBAC — topics returned are only what user can access
  ...
  // Backend already filters by RBAC
  ...
  return (
    <div className="page-enter">
      <Header title={showTrash ? 'Papelera de Temas' : 'Temas'} actions={
        <div className="flex gap-2">
          {canDelete && (
            <button onClick={() => { setShowTrash(!showTrash); if (!showTrash) loadTrashedTopics(); }}
              className="flex items-center gap-2 px-4 py-2 text-sm rounded-lg border font-semibold transition-colors"
              style={{ borderColor: showTrash ? '#c53030' : TJ.border, color: showTrash ? '#c53030' : '#a8b8d8', fontFamily: 'Montserrat, sans-serif' }}>
              <Archive size={15} /> {showTrash ? 'Ver Temas' : 'Papelera'}
            </button>
          )}
          {canCreate && !showTrash && (<>
        ...
        {showTrash ? (
          <div className="space-y-2">
            {trashedTopics.length === 0 ? (
              ...
              <div key={topic.id}
                className="bg-white rounded-xl p-3 md:p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-2"
                style={{ border: '1px solid #fed7d7', background: '#fff5f5' }}>
                <div className="flex-1 min-w-0">
                  <span className="font-semibold text-sm" style={{ color: TJ.text, fontFamily: 'Montserrat, sans-serif' }}>{topic.title}</span>
                  <div className="text-xs mt-1" style={{ color: '#a8b8d8' }}>{topic.author} · {topic.offerName || 'Sin oferta'}</div>
                </div>
                <div className="flex items-center gap-2">
                  <button onClick={() => restoreTopic(topic.id)}
                    className="flex items-center gap-1 px-3 py-1.5 text-xs rounded-lg border transition-colors"
                    style={{ borderColor: '#c6f6d5', color: '#276749' }}
                    onMouseEnter={e => (e.currentTarget.style.background = '#f0fff4')}
                    onMouseLeave={e => (e.currentTarget.style.background = 'transparent')}>
                    <RotateCcw size={12} /> Restaurar
                  </button>
                  {currentUser.role === 'admin' && (
                    <button onClick={() => { if (confirm('¿Eliminar permanentemente? Esta acción no se puede deshacer.')) permanentDeleteTopic(topic.id); }}
                      className="flex items-center gap-1 px-3 py-1.5 text-xs rounded-lg border transition-colors"
                                  ...
                                  <FileText size={16} />
                                            ...
                                            {canDelete && (
                                              <button onClick={(e) => { e.stopPropagation(); if (confirm('¿Enviar este tema a la papelera?')) deleteTopic(topic.id); }}
                                                className="p-2 rounded-lg hover:bg-red-50 transition-colors" style={{ color: '#c53030' }} title="Eliminar">
                                                <Trash2 size={16} />
                                              </button>
                                            )}
                                          </div>
