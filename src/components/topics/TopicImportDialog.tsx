...
export default function TopicImportDialog({ open, onClose, offers, currentUserId, currentUserName, onImport, onCreateOffer }: Props) {
  ...
  const handleFiles = useCallback((files: FileList) => {
    ...
    const finishFile = (parsed: ParsedTopic[]) => {
        ...
        file.arrayBuffer().then(async (arrayBuffer) => {
          try {
            ...
            const result = await mammoth.convertToHtml(
              { arrayBuffer },
              {
                convertImage: mammoth.images.imgElement(function(image: { contentType: string; read: (encoding: string) => Promise<string> }) {
            ...
            const parsed = parseHTMLToTopics(result.value);
            finishFile(parsed);
          } catch (err) {
            ...
            reader.onload = () => {
              const parsed = parseHTMLToTopics(reader.result as string);
              finishFile(parsed);
            };
